<?php
/**
 * Plugin Name: Reg-Pro drag and drop
 * Plugin URI: ayaz.khorajia@gmail.com
 * Description: Easy registration form and profile edit.
 * Version: 1.0
 * Author: aiyaz
...
function rpdd_admin_enqueue() {
    ...
    $rpdd_ajax = array('form_fields' => $form_fields, 'ajax_url' =>admin_url('admin-ajax.php') );
...
function rpdd_create_sub_menu() {
	add_menu_page( 'Reg-pro', 'Reg-pro', 'manage_options', 'rpdd-setting-page', 'rpdd_settings_page' );
}
...
function rpdd_register_shortcode_function() { 
    ...
    foreach( $form_fields as $field ){
      ...
      if( $data_type === 'date' ) {
        $label = $field->label;
        $required = $field->required;
        $html .= '<div class="form-group"><label class="control-label">' . $label  .'</label><input type="date" name="' . $name . '" placeholder="' . $placeholder . '" class="form-control" ' . $required . '/></div>';
      }
      if( $data_type === 'radio' ) {
      ...
      if( $data_type === 'checkbox' ) {
        ...
        $option_html = '';
        foreach($options as $option){
          $option_html .= '<div class="form-check"><label class="form-check-label"><input type="checkbox" class="form-check-input" name="' . $name .'[]" value="' . $option->value . '">' . $option->label . '</label></div>';
        }
        $html .= '<div class="form-group"><label class="control-label">' . $label . '</label>' . $option_html . '</div>';
      }
...
function save_regpro_settings(){
    ...
    if( isset( $_POST['form_fields'] ) &&  ! empty( $_POST['form_fields'] )  ){
        update_option( 'reg-pro-registration-fields', json_encode( $_POST['form_fields'] ) );
        echo true;
    } else{
      echo false;
    }
